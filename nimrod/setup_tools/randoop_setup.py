...
class Randoop_setup:
    ...
    def exec_randoop_jar_test_conflict_second_criteria(self, evo, scenario, jarBase, jarParentLeft, jarParentRight, jarMerge, commitBaseSha, commitParentLeft, commitParentRight, commitMergeSha):
        conflict_info = []
        try:
            self.setup_for_merge_scenario_second(evo, scenario, jarBase, jarParentLeft, jarParentRight, jarMerge)
            self.exec_randoop_test_conflict_second(evo, scenario, commitBaseSha, commitParentLeft, commitParentLeft, commitMergeSha, conflict_info)
    ...
    def exec_randoop_for_both_criteria(self, evo, scenario, jarBase, jarParentLeft, jarParentRight, jarMerge, commitBaseSha, commitParentLeft, commitParentRight, commitMergeSha):
        conflict_info = []
        try:
            self.setup_for_merge_scenario_second(evo, scenario, jarBase, jarParentLeft, jarParentRight, jarMerge)
            self.exec_randoop_test_conflict_second(evo, scenario, commitBaseSha, commitParentLeft, commitParentLeft, commitMergeSha, conflict_info)
    ...
    #def exec_randoop_test_conflict_second(self, evo, scenario, commitBaseSha, commitParentLeft, commitParentRight, commitMergeSha, conflict_info):
    def exec_randoop_test_conflict_second(self, evo, scenario, commitBaseSha, commitParentTestSuite, commitOtherParent, commitMergeSha, conflict_info):
        try:
            path_suite = self.gen_randoop(scenario, evo.project_dep)

            test_result_base = self.try_randoop(evo.project_dep.classes_dir, evo.project_dep.sut_class, evo.project_dep.dRegCp, evo.project_dep)  # fail on base - passing tests 0 2 7
            test_result_parent_test_suite = self.try_randoop(evo.project_dep.classes_dir, evo.project_dep.sut_class, evo.project_dep.classes_dir, evo.project_dep)  # pass on left
            test_result_other_parent = self.try_randoop(evo.project_dep.classes_dir, evo.project_dep.sut_class, evo.project_dep.rightDir, evo.project_dep)  # pass on left
            test_result_merge = self.try_randoop(evo.project_dep.classes_dir, evo.project_dep.sut_class, evo.project_dep.mergeDir, evo.project_dep)  # fail on merge - passing tests 0 2 7

            conflict_info.append(self.behaviour_change.check_different_test_results_for_merge_scenario(test_result_base, test_result_parent_test_suite, test_result_merge, path_suite, commitBaseSha, commitParentTestSuite, commitMergeSha, "randoop"))
            conflict_info.append(self.behaviour_change.check_different_test_results_for_merge_scenario_second_criterion(test_result_base, test_result_parent_test_suite, test_result_other_parent, test_result_merge, path_suite, commitBaseSha, commitParentTestSuite, commitOtherParent, commitMergeSha, "randoop"))

        except:
            print("Some project versions could not be evaluated")
            conflict_info.append(["NONE", set(), "NO-INFORMATION", commitBaseSha, commitParentTestSuite, commitMergeSha, "randoop"])
