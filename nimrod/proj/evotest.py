    ...
    with open(config['path_hash_csv']) as csv_file:
        ...
        for row in csv_reader:
            if row[1] == "commit_pairs":
                ...
                evosuite_diff_commit_pair = evo.evosuite_diff_setup.exec_evosuite_diff_jar_commit_pair(evo, merge, row[6], row[7], row[2], row[3])
                evo.output_report.write_output_results_commit_pairs(row[0], evosuite_diff_commit_pair, row[4], row[5])
                evosuite_commit_pair = evo.evosuite_setup.exec_evosuite_jar_commit_pair(evo, merge, row[6], row[7],row[2], row[3])
                evo.output_report.write_output_results_commit_pairs(row[0], evosuite_commit_pair, row[4], row[5])
                randoop_commit_pair = evo.randoop_setup.exec_randoop_jar_commit_pair(evo, merge, row[6], row[7],row[2], row[3])
